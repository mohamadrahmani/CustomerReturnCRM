...
const navigation = [
  ...
  { href: "/sms-templates", label: "قالب‌های SMS", icon: "sms" },
  { href: "/sms-campaigns", label: "تاریخچه SMS", icon: "history" },
];
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 md:flex">
      <aside className="hidden w-64 shrink-0 bg-slate-950 px-4 py-6 text-white md:flex md:flex-col">
        ...
        <div className="mt-auto border-t border-white/10 pt-4">
          <button
            ...
            className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-right text-sm text-slate-400 hover:bg-white/5 hover:text-white"
          >
      ...
      <aside
        className={`fixed inset-y-0 right-0 z-40 w-72 bg-slate-950 px-4 py-5 text-white shadow-2xl transition-transform md:hidden ${
          mobileOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="flex items-start justify-between">
          <Brand />
          <button onClick={() => setMobileOpen(false)} className="rounded-lg px-2 py-1 text-slate-400">
            ×
          </button>
        </div>
        ...
        <button
          onClick={handleLogout}
          className="mt-8 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-right text-sm text-slate-400 hover:bg-white/5 hover:text-white"
        >
          <Icon name="logout" className="h-4 w-4" />
          خروج از حساب
        </button>
      ...
      <div className="min-w-0 flex-1">
          ...
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              <button
                ...
                className="order-2 flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-600 md:hidden"
              ...
              <div className="min-w-0">
                ...
                {auth?.businesses.length ? (
                  <select
                    ...
                    value={businessId ?? auth.businesses[0].id}
                    onChange={(e) => setBusinessId(e.target.value)}
                    className="mt-0.5 max-w-[220px] bg-transparent text-sm font-extrabold text-slate-800 outline-none"
                  >
                  ...
                  <p className="text-sm font-semibold">بدون کسب‌وکار</p>
                )}
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={handleLogout}
                className="hidden rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-600 md:inline-flex"
              >
                خروج
              </button>
            </div>
        ...
        <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-4 border-t border-slate-200 bg-white/95 pb-safe shadow-[0_-4px_20px_rgba(15,23,42,0.04)] backdrop-blur md:hidden">
          ...
          <button
            onClick={() => setMobileOpen(true)}
            className="flex min-h-[58px] flex-col items-center justify-center gap-1 text-[11px] font-medium text-slate-500"
          >
            <Icon name="menu" className="h-5 w-5" />
            <span>بیشتر</span>
          </button>
...
function Brand() {
  return (
    <div>
      <p className="text-sm font-bold tracking-tight text-white">Customer Return CRM</p>
      <p className="mt-1 text-xs text-slate-500">مدیریت مشتری و بازگشت</p>
    </div>
  );
}
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${
        active
          ? "bg-indigo-500/15 font-semibold text-white ring-1 ring-indigo-400/10"
...
function BottomNavItem({ item, active }: { item: (typeof navigation)[number]; active: boolean }) {
  return (
    <Link
      href={item.href}
...
function Icon({ name, className = "h-5 w-5" }: { name: string; className?: string }) {
  ...
  if (name === "sms") return <svg {...common}><path d="M5 5h14a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H9l-4 4v-4a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z"/><path d="M7 9h10M7 13h6"/></svg>;
  if (name === "history") return <svg {...common}><path d="M3 12a9 9 0 1 0 3-6.7"/><path d="M3 4v5h5M12 7v5l3 2"/></svg>;
