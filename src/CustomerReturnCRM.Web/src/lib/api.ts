...
export type Reminder = { id: string; businessId: string; customerId: string; customerName: string; customerMobile: string | null; serviceId: string | null; serviceTitle: string | null; title: string; dueAt: string; status: number; note: string | null; createdByUserId: string; completedAt: string | null; createdAt: string; updatedAt: string | null };
...
export async function completeReminder(businessId: string, reminderId: string, note = "") { return apiFetch<Reminder>(`/api/businesses/${businessId}/reminders/${reminderId}/complete`, { method: "POST", body: JSON.stringify({ note: note.trim() || undefined }) }); }
