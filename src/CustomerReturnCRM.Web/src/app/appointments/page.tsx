...
function statusClass(s: number) { return s === 1 ? "border-indigo-200 bg-indigo-50 text-indigo-700" : s === 2 ? "border-emerald-200 bg-emerald-50 text-emerald-700" : s === 3 ? "border-slate-200 bg-slate-100 text-slate-500" : "border-amber-200 bg-amber-50 text-amber-700"; }
...
function JalaliDatePicker({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return <div className="relative">
    <button type="button" onClick={() => setOpen(v => !v)} className="crm-input flex w-full items-center justify-between text-right"><span className="font-semibold">{new Intl.DateTimeFormat("fa-IR-u-ca-persian", { day: "numeric", month: "long", year: "numeric" }).format(current)}</span><span className="text-slate-400">▦</span></button>
    {open && <><button type="button" aria-label="بستن تقویم" className="fixed inset-0 z-30" onClick={() => setOpen(false)} /><div className="absolute right-0 top-full z-40 mt-2 w-[320px] rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl">
      <div className="mb-3 flex items-center justify-between"><button type="button" onClick={() => setMonth(m => shiftJalaliMonth(m, -1))} className="rounded-lg px-3 py-1.5 hover:bg-slate-100">‹</button><b className="text-sm">{jalaliMonthNames[month.month - 1]} {faNumber.format(month.year)}</b><button type="button" onClick={() => setMonth(m => shiftJalaliMonth(m, 1))} className="rounded-lg px-3 py-1.5 hover:bg-slate-100">›</button></div>
      ...
      <div className="grid grid-cols-7 gap-1">{cells.map((day, i) => day === null ? <span key={`e-${i}`} /> : <button type="button" key={day} onClick={() => { onChange(jalaliToIso({ year: month.year, month: month.month, day })); setOpen(false); }} className={`h-9 rounded-lg text-xs font-bold ${day === selected.day && month.month === selected.month && month.year === selected.year ? "bg-pink-500 text-white" : day === today.day && month.month === today.month && month.year === today.year ? "bg-pink-50 text-pink-600" : "text-slate-700 hover:bg-slate-100"}`}>{faNumber.format(day)}</button>)}</div>
      <button type="button" onClick={() => { onChange(jalaliToIso(today)); setOpen(false); }} className="mt-3 w-full rounded-xl bg-slate-50 py-2 text-xs font-bold text-pink-600 hover:bg-pink-50">امروز</button>
    </div></>}
  </div>;
}
...
function CustomerPicker({ customers, value, onChange }: { customers: Customer[]; value: string; onChange: (id: string) => void }) {
  ...
  return <div className="relative">
    <button type="button" onClick={() => setOpen(v => !v)} className="crm-input flex w-full items-center justify-between text-right"><span className={selected ? "text-slate-900" : "text-slate-400"}>{selected ? customerName(selected) : "انتخاب مشتری"}</span><span className="text-slate-400">⌄</span></button>
    {open && <><button type="button" aria-label="بستن انتخاب مشتری" className="fixed inset-0 z-30" onClick={() => setOpen(false)} /><div className="absolute right-0 top-full z-40 mt-2 w-full min-w-[320px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
...
export default function AppointmentsPage() {
  ...
  async function createAppointment() {
    const service = services.data?.items.find(s => s.id === form.serviceId);
    ...
    try {
      await apiFetch(`/api/businesses/${activeBusinessId}/appointments`, { method: "POST", body: JSON.stringify({ customerId: form.customerId, startAt: localDateTime(form.date, form.start), endAt: localDateTime(form.date, form.end), note: form.note.trim() || undefined, services: [{ serviceId: service.id, staffId: form.staffId }] }) });
      setModal(null);
      await queryClient.invalidateQueries({ queryKey: ["appointments", activeBusinessId] });
