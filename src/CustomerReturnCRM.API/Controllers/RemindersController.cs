...
{
    [HttpGet]
    public async Task<ActionResult<IReadOnlyList<ReminderResult>>> List(
        Guid businessId,
        ...
        [FromServices] IReminderManagementService service = null!,
        CancellationToken cancellationToken = default)
    {
        if (!TryGetUserId(out var userId)) return Unauthorized();
        try
        {
            return Ok(await service.ListAsync(businessId, userId, status, from, to, page, pageSize, cancellationToken));
        }
        catch (UnauthorizedAccessException) { return Forbid(); }
    }

    [HttpGet("{reminderId:guid}")]
    public async Task<ActionResult<ReminderResult>> Get(
        Guid businessId,
        Guid reminderId,
        [FromServices] IReminderManagementService service,
        CancellationToken cancellationToken)
    {
        if (!TryGetUserId(out var userId)) return Unauthorized();
        try
        {
            var result = await service.GetAsync(businessId, reminderId, userId, cancellationToken);
    ...
    [HttpPost]
    public async Task<ActionResult<ReminderResult>> Create(
        Guid businessId,
    ...
    public Task<IActionResult> Complete(
        ...
        CancellationToken cancellationToken) =>
        ChangeStatusAsync(
            (userId, token) => service.CompleteAsync(businessId, reminderId, userId, token),
            cancellationToken);
    ...
    public Task<IActionResult> Cancel(
        ...
        CancellationToken cancellationToken) =>
        ChangeStatusAsync(
            (userId, token) => service.CancelAsync(businessId, reminderId, userId, token),
            cancellationToken);

    private async Task<IActionResult> ChangeStatusAsync(
        Func<Guid, CancellationToken, Task<ReminderResult?>> operation,
        CancellationToken cancellationToken)
    {
        if (!TryGetUserId(out var userId)) return Unauthorized();
        try
