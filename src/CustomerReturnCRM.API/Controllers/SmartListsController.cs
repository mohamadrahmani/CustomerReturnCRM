...
public sealed class SmartListsController : ControllerBase
{
    ...
    public Task<ActionResult<PagedResult<SmartListItemResult>>> GetOverdue(
        ...
        [FromServices] IReturnAnalysisService service = null!,
        CancellationToken cancellationToken = default) =>
        ExecuteAsync(
            (userId, token) => service.GetOverdueAsync(businessId, userId, page, pageSize, token),
            cancellationToken);
    ...
    public Task<ActionResult<PagedResult<SmartListItemResult>>> GetDueSoon(
        ...
        CancellationToken cancellationToken = default) =>
        ExecuteAsync(
            (userId, token) => service.GetDueSoonAsync(businessId, userId, page, pageSize, token),
            cancellationToken);
    ...
    public Task<ActionResult<PagedResult<SmartListItemResult>>> GetAtRisk(
        ...
        CancellationToken cancellationToken = default) =>
        ExecuteAsync(
            (userId, token) => service.GetAtRiskAsync(businessId, userId, page, pageSize, token),
            cancellationToken);
    ...
    public Task<ActionResult<PagedResult<SmartListItemResult>>> GetNoRecentVisit(
        ...
        CancellationToken cancellationToken = default) =>
        ExecuteAsync(
            (userId, token) => service.GetNoRecentVisitAsync(businessId, userId, page, pageSize, token),
            cancellationToken);
    ...
    public Task<IActionResult> Dismiss(
        ...
        ExecuteMutationAsync(
            (userId, token) => service.DismissAsync(businessId, userId, request, token),
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            return Ok(await query(userId, cancellationToken));
        }
    ...
    private async Task<IActionResult> ExecuteMutationAsync(
        Func<Guid, CancellationToken, Task<bool>> mutation,
        CancellationToken cancellationToken)
    {
        if (!TryGetUserId(out var userId))
        {
        ...
        try
        {
            return await mutation(userId, cancellationToken) ? NoContent() : NotFound();
        }
