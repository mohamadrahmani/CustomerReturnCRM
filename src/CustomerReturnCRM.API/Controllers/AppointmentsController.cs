...
public sealed class AppointmentsController : ControllerBase
{
    ...
    public async Task<ActionResult<AppointmentResult>> NoShow(Guid businessId,Guid appointmentId,[FromServices]IAppointmentManagementService service,CancellationToken cancellationToken){if(!TryGetUserId(out var userId))return Unauthorized();try{var result=await service.MarkNoShowAsync(businessId,appointmentId,userId,cancellationToken);return result is null?NotFound():Ok(result);}catch(InvalidOperationException e){return Conflict(new{error=e.Message});}catch(UnauthorizedAccessException){return Forbid();}}
