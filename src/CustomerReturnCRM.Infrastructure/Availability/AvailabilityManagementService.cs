...
public sealed class AvailabilityManagementService : IAvailabilityManagementService
{
    ...
    public async Task<IReadOnlyList<WorkingHourResult>> GetBusinessWorkingHoursAsync(Guid businessId, Guid userId, CancellationToken ct = default)
    {
        ...
        return await _db.BusinessWorkingHours.AsNoTracking().Where(x => x.BusinessId == businessId).OrderBy(x => x.DayOfWeek).ThenBy(x => x.StartTime).Select(ToResult).ToListAsync(ct);
    }
    ...
    public async Task<IReadOnlyList<WorkingHourResult>> ReplaceBusinessWorkingHoursAsync(Guid businessId, Guid userId, IReadOnlyCollection<WorkingHourRequest> hours, CancellationToken ct = default)
    {
        await EnsureMemberAsync(businessId, userId, ct);
        ValidateHours(hours);
        var existing = await _db.BusinessWorkingHours.Where(x => x.BusinessId == businessId).ToListAsync(ct);
        _db.BusinessWorkingHours.RemoveRange(existing);
        _db.BusinessWorkingHours.AddRange(hours.Select(x => new BusinessWorkingHour { Id = Guid.NewGuid(), BusinessId = businessId, DayOfWeek = x.DayOfWeek, StartTime = x.StartTime, EndTime = x.EndTime }));
    ...
    public async Task<IReadOnlyList<WorkingHourResult>> GetStaffWorkingHoursAsync(Guid businessId, Guid staffId, Guid userId, CancellationToken ct = default)
    {
        ...
        return await _db.StaffWorkingHours.AsNoTracking().Where(x => x.StaffId == staffId).OrderBy(x => x.DayOfWeek).ThenBy(x => x.StartTime).Select(ToResult).ToListAsync(ct);
    }
    ...
    public async Task<IReadOnlyList<WorkingHourResult>> ReplaceStaffWorkingHoursAsync(Guid businessId, Guid staffId, Guid userId, IReadOnlyCollection<WorkingHourRequest> hours, CancellationToken ct = default)
    {
        ...
        _db.StaffWorkingHours.AddRange(hours.Select(x => new StaffWorkingHour { Id = Guid.NewGuid(), StaffId = staffId, DayOfWeek = x.DayOfWeek, StartTime = x.StartTime, EndTime = x.EndTime }));
        await _db.SaveChangesAsync(ct);
    ...
    public async Task<IReadOnlyList<StaffTimeOffResult>> ListTimeOffAsync(Guid businessId, Guid staffId, Guid userId, DateTime from, DateTime to, CancellationToken ct = default)
    {
        ...
        return await _db.StaffTimeOffs.AsNoTracking().Where(x => x.StaffId == staffId && x.EndAt > from && x.StartAt < to).OrderBy(x => x.StartAt).Select(x => new StaffTimeOffResult(x.Id, x.StaffId, x.StartAt, x.EndAt, x.Reason)).ToListAsync(ct);
    }
    ...
    {
        await EnsureStaffAsync(businessId, staffId, userId, ct);
        var item = await _db.StaffTimeOffs.SingleOrDefaultAsync(x => x.Id == timeOffId && x.StaffId == staffId, ct);
        if (item is null) return false;
        _db.StaffTimeOffs.Remove(item);
        await _db.SaveChangesAsync(ct);
        return true;
    }
    ...
    public async Task<IReadOnlyList<AvailabilitySlotResult>> GetAvailableSlotsAsync(Guid businessId, AvailabilityRequest request, CancellationToken ct = default)
    {
        ...
        var business = await _db.Businesses.AsNoTracking().SingleOrDefaultAsync(x => x.Id == businessId && x.IsActive, ct) ?? throw new ArgumentException("Business was not found or is inactive.");
        ...
        var businessHours = await _db.BusinessWorkingHours.AsNoTracking().Where(x => x.BusinessId == businessId && x.DayOfWeek == day).OrderBy(x => x.StartTime).ToListAsync(ct);
        ...
        var appointments = await _db.Appointments.AsNoTracking().Where(x => x.BusinessId == businessId && x.StartAt < dayEnd && x.EndAt > dayStart && x.Status != AppointmentStatus.Cancelled && x.Status != AppointmentStatus.NoShow).Select(x => new { x.StartAt, x.EndAt, Services = x.AppointmentServices.Select(s => s.StaffId) }).ToListAsync(ct);
        var timeOffs = await _db.StaffTimeOffs.AsNoTracking().Where(x => staffIds.Contains(x.StaffId) && x.StartAt < dayEnd && x.EndAt > dayStart).ToListAsync(ct);
        var staffHours = await _db.StaffWorkingHours.AsNoTracking().Where(x => staffIds.Contains(x.StaffId) && x.DayOfWeek == day).OrderBy(x => x.StartTime).ToListAsync(ct);

        var result = new List<AvailabilitySlotResult>();
        foreach (var staffMember in staff)
        {
            var configuredStaffHours = staffHours.Where(x => x.StaffId == staffMember.Id).ToList();
            var effectiveHours = configuredStaffHours.Count == 0
                ? businessHours.Select(x => (x.StartTime, x.EndTime)).ToList()
                : configuredStaffHours.Select(x => (x.StartTime, x.EndTime)).ToList();

            foreach (var businessHour in businessHours)
            foreach (var staffHour in effectiveHours)
            {
                ...
                for (var start = rangeStart; start.AddMinutes(service.DefaultDurationMinutes) <= rangeEnd; start = start.AddMinutes(request.SlotIntervalMinutes))
                {
                    ...
                    if (appointments.Any(a => a.Services.Contains(staffMember.Id) && a.StartAt < end && a.EndAt > start)) continue;
    ...
    {
        await EnsureMemberAsync(businessId, userId, ct);
        if (!await _db.Staff.AnyAsync(x => x.Id == staffId && x.BusinessId == businessId && x.IsActive, ct)) throw new ArgumentException("Staff member was not found or is inactive.");
    }
    ...
    private static void ValidateHours(IReadOnlyCollection<WorkingHourRequest> hours)
    {
        foreach (var group in hours.GroupBy(x => x.DayOfWeek))
        {
            ...
            foreach (var hour in ordered)
                if (hour.StartTime < TimeSpan.Zero || hour.EndTime > TimeSpan.FromDays(1) || hour.EndTime <= hour.StartTime)
                    throw new ArgumentException("Working-hour intervals must be within the day and have an end after their start.");
            for (var i = 1; i < ordered.Count; i++)
                if (ordered[i].StartTime < ordered[i - 1].EndTime)
                    throw new ArgumentException("Working-hour intervals cannot overlap.");
        }
    }
