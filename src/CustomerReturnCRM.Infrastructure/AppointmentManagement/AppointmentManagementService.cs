...
public sealed class AppointmentManagementService : IAppointmentManagementService
{
    ...
    public async Task<AppointmentResult> CreateAsync(Guid businessId, Guid userId, CreateAppointmentRequest request, CancellationToken cancellationToken = default)
    {
        ...
        await EnsureNoStaffOverlapAsync(businessId, request.StartAt, request.EndAt, request.Services.Select(x => x.StaffId), null, cancellationToken);
        var appointment = new Appointment { Id=Guid.NewGuid(), BusinessId=businessId, CustomerId=request.CustomerId, StartAt=request.StartAt, EndAt=request.EndAt, Status=request.Status, Note=Normalize(request.Note), CreatedAt=DateTime.UtcNow };
        AddServices(appointment, request.Services, snapshots); _dbContext.Appointments.Add(appointment); await _dbContext.SaveChangesAsync(cancellationToken); return ToResult(appointment);
    }
    ...
    {
        await EnsureMemberAsync(businessId,userId,cancellationToken); var query=_dbContext.Appointments.AsNoTracking().Include(x=>x.AppointmentServices).Where(x=>x.BusinessId==businessId);
        if(from.HasValue) query=query.Where(x=>x.StartAt>=from.Value); if(to.HasValue) query=query.Where(x=>x.StartAt<to.Value); (page,pageSize)=Pagination.Normalize(page,pageSize); var total=await query.CountAsync(cancellationToken); var appointments=await query.OrderBy(x=>x.StartAt).Skip((page-1)*pageSize).Take(pageSize).ToListAsync(cancellationToken); return Pagination.Create(appointments.Select(ToResult).ToList(),page,pageSize,total);
    }
    ...
    { await EnsureMemberAsync(businessId,userId,cancellationToken); var appointment=await _dbContext.Appointments.AsNoTracking().Include(x=>x.AppointmentServices).SingleOrDefaultAsync(x=>x.BusinessId==businessId&&x.Id==appointmentId,cancellationToken); return appointment is null?null:ToResult(appointment); }
    ...
    {
        await EnsureMemberAsync(businessId,userId,cancellationToken); Validate(request.StartAt,request.EndAt,request.Status,request.Services); var appointment=await _dbContext.Appointments.Include(x=>x.AppointmentServices).SingleOrDefaultAsync(x=>x.BusinessId==businessId&&x.Id==appointmentId,cancellationToken); if(appointment is null)return null; if(appointment.Status==AppointmentStatus.Completed)throw new InvalidOperationException("Completed appointments cannot be updated."); var snapshots=await LoadAndValidateReferencesAsync(businessId,request.CustomerId,request.Services,cancellationToken); await EnsureNoStaffOverlapAsync(businessId,request.StartAt,request.EndAt,request.Services.Select(x=>x.StaffId),appointmentId,cancellationToken); appointment.CustomerId=request.CustomerId; appointment.StartAt=request.StartAt; appointment.EndAt=request.EndAt; appointment.Status=request.Status; appointment.Note=Normalize(request.Note); appointment.UpdatedAt=DateTime.UtcNow; _dbContext.AppointmentServices.RemoveRange(appointment.AppointmentServices); appointment.AppointmentServices.Clear(); AddServices(appointment,request.Services,snapshots); await _dbContext.SaveChangesAsync(cancellationToken); return ToResult(appointment);
    }

    public async Task<AppointmentResult?> ConfirmAsync(Guid businessId,Guid appointmentId,Guid userId,CancellationToken cancellationToken=default)
    { await EnsureMemberAsync(businessId,userId,cancellationToken); var appointment=await _dbContext.Appointments.Include(x=>x.AppointmentServices).SingleOrDefaultAsync(x=>x.BusinessId==businessId&&x.Id==appointmentId,cancellationToken); if(appointment is null)return null; if(appointment.Status==AppointmentStatus.Cancelled)throw new InvalidOperationException("Cancelled appointments cannot be confirmed."); if(appointment.Status==AppointmentStatus.Completed)throw new InvalidOperationException("Completed appointments cannot be confirmed."); appointment.Status=AppointmentStatus.Confirmed; appointment.UpdatedAt=DateTime.UtcNow; await _dbContext.SaveChangesAsync(cancellationToken); return ToResult(appointment); }
    ...
    {
        await EnsureMemberAsync(businessId,userId,cancellationToken); var appointment=await _dbContext.Appointments.Include(x=>x.AppointmentServices).SingleOrDefaultAsync(x=>x.BusinessId==businessId&&x.Id==appointmentId,cancellationToken); if(appointment is null)return null; if(appointment.Status==AppointmentStatus.Completed)throw new InvalidOperationException("Completed appointments cannot be marked as no-show."); if(appointment.Status==AppointmentStatus.Cancelled)throw new InvalidOperationException("Cancelled appointments cannot be marked as no-show."); appointment.Status=AppointmentStatus.NoShow; appointment.UpdatedAt=DateTime.UtcNow; await _dbContext.SaveChangesAsync(cancellationToken); return ToResult(appointment);
    }
    ...
    private async Task EnsureNoStaffOverlapAsync(Guid businessId, DateTime startAt, DateTime endAt, IEnumerable<Guid> staffIds, Guid? excludedAppointmentId, CancellationToken cancellationToken)
    {
    ...
    private static void Validate(DateTime startAt,DateTime endAt,AppointmentStatus status,IReadOnlyCollection<AppointmentServiceRequest> services){if(endAt<=startAt)throw new ArgumentException("End time must be after start time."); if(status==AppointmentStatus.Completed)throw new ArgumentException("Appointment completion is not available in this phase."); if(services.Count==0)throw new ArgumentException("An appointment requires at least one service."); if(services.Any(x=>x.ServiceId==Guid.Empty||x.StaffId==Guid.Empty))throw new ArgumentException("Service and staff are required for every appointment service."); if(services.Select(x=>x.ServiceId).Distinct().Count()!=services.Count)throw new ArgumentException("An appointment cannot contain the same service more than once.");}
