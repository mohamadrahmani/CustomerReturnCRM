...
public sealed class ReminderManagementService : IReminderManagementService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var query = _dbContext.Reminders.AsNoTracking()
    ...
    public async Task<ReminderResult?> GetAsync(
        Guid businessId,
        Guid reminderId,
        Guid userId,
        CancellationToken cancellationToken = default)
    ...
    public async Task<ReminderResult> CreateAsync(
        Guid businessId,
        Guid userId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!await _dbContext.Customers.AnyAsync(
                x => x.BusinessId == businessId && x.Id == request.CustomerId && x.IsActive,
                cancellationToken))
        ...
        if (request.ServiceId.HasValue &&
            !await _dbContext.Services.AnyAsync(
                x => x.BusinessId == businessId && x.Id == request.ServiceId.Value && x.IsActive,
                cancellationToken))
    ...
    public Task<ReminderResult?> CompleteAsync(
        Guid businessId,
        Guid reminderId,
        Guid userId,
        CancellationToken cancellationToken = default) =>
        ChangeStatusAsync(businessId, reminderId, userId, ReminderStatus.Completed, cancellationToken);

    public Task<ReminderResult?> CancelAsync(
        Guid businessId,
        Guid reminderId,
        Guid userId,
        CancellationToken cancellationToken = default) =>
        ChangeStatusAsync(businessId, reminderId, userId, ReminderStatus.Cancelled, cancellationToken);
    ...
    {
        await EnsureMemberAsync(businessId, userId, cancellationToken);
        var reminder = await _dbContext.Reminders.SingleOrDefaultAsync(
            x => x.BusinessId == businessId && x.Id == reminderId,
            cancellationToken);
    ...
    private static ReminderResult ToResult(Reminder x) =>
        new(x.Id, x.BusinessId, x.CustomerId, x.ServiceId, x.Title, x.DueAt, x.Status,
            x.Note, x.CreatedByUserId, x.CompletedAt, x.CreatedAt, x.UpdatedAt);
}
